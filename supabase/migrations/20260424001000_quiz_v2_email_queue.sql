-- Quiz V2 — Email queue + scheduling for Resend-based sends (bypass Encharge UI blocks)
-- One submission produces 5 rows (M*.1 à M*.5) scheduled at created_at + {0d, 2d, 5d, 9d, 12d}.
-- n8n cron worker scans toutes les heures → envoie via Resend API → marque sent_at.
...
CREATE TABLE IF NOT EXISTS public.quiz_v2_email_queue (
  ...
  mail_code TEXT NOT NULL,                  -- 'M1.1' … 'M6.5'
  day_delay INT NOT NULL,                   -- 0 / 2 / 5 / 9 / 12
...
-- Zero policies → no client access. Only service_role + SECURITY DEFINER RPCs.

-- ────────────────────────────────────────────────────────────────────
-- RPC 1: enqueue 5 emails for a fresh submission. Called by the Edge
-- function "forward-to-n8n" after the n8n webhook succeeds.
...
BEGIN
  ...
  -- Map archetype slug → sequence number (1-6) to derive M1.x, M2.x, etc.
...
-- Service role access only (via edge function).

-- ────────────────────────────────────────────────────────────────────
-- RPC 2: get ready-to-send emails. Called by n8n cron (every hour).
-- Limit to avoid mass sends in case of queue backlog.
...
AS $$
  ...
  WHERE q.send_at <= now()
    ...
    -- Skip if this email has unsubscribed from a previous send
    AND NOT EXISTS (
...
-- ────────────────────────────────────────────────────────────────────
-- RPC 3: mark email as sent successfully.
...
-- ────────────────────────────────────────────────────────────────────
-- RPC 4: mark email send as failed (network error, Resend 4xx/5xx).
...
-- ────────────────────────────────────────────────────────────────────
-- RPC 5: unsubscribe by token. Called by n8n /unsub endpoint.
-- Flags ALL rows for that email (future + already-sent) so no more emails leave.
...
-- ^ anon OK: the token IS the auth (24 random bytes, unguessable). Safe to call from frontend/n8n without further auth.

-- ────────────────────────────────────────────────────────────────────
-- RPC 6: admin stats for beta dashboard.
