-- Quiz V2 — Email templates storage (30 rows, one per archetype × mail).
-- Seeded once from the Encharge backup JSON (cf. seed-email-templates.js).
-- Allows the n8n worker to SELECT the HTML+subject by mail_code instead of
-- embedding 90 kB of templates inline in the workflow.

CREATE TABLE IF NOT EXISTS public.quiz_v2_email_templates (
  mail_code TEXT NOT NULL PRIMARY KEY,     -- 'M1.1' … 'M6.5'
...
-- No policies → only service_role + SECURITY DEFINER RPC can access.

-- RPC: fetch template by mail_code (called by n8n worker)
